...
async function updateHourlyForecast(city) {
  ...
  try {
    const geocodeResponse = await fetch(
      `https://api.openweathermap.org/geo/2.5/direct?q=${city}&limit=1&appid=${apiKey}`
    );
    const geoData = await geocodeResponse.json();

    if (!geoData.length) {
      console.error("Не знайдено координат для міста:", city);
      return;
    }
    ...
    if (!hourlyItems) {
      console.warn('Елемент .hourly-items не знайдено в DOM');
      return;
    }
    ...
    console.error('Помилка при оновленні погодинного прогнозу:', error);
  }
}
